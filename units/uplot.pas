...
const
  ...
  CurvColor: array [1 .. MaxCurvColor] of Integer = (12, { LightRed }
    14, { Yellow }
    10, { LightGreen }
    9, { LightBlue }
    11, { LightCyan }
...
begin
  ...
  begin
    ...
    SetUserCharSize(YFont.Width, 100, YFont.Height, 100);
    OutTextXY(XminPixel - GetMaxX div 8, (YminPixel + YmaxPixel) div 2, YTitle);
  end;
end;
...
begin
  ...
  OutTextXY((XminPixel + XmaxPixel) div 2, YminPixel - GetMaxY div 10,
    GraphTitle);
end;
...
begin
  ...
  XmaxPixel := XmaxPixel - XminPixel;
  XminPixel := 0;
  YmaxPixel := YmaxPixel - YminPixel;
  YminPixel := 0;
end;
...
begin
  ...
  case CurvParam^[CurvIndex].PointParam.Symbol of
    ...
    1:
      PieSlice(Xp, Yp, 0, 360, Dx); { Solid circle }
    2:
      Ellipse(Xp, Yp, 0, 360, Dx, Dy); { Open circle }
    3:
      FillPoly(5, Square); { Solid square }
    4:
      DrawPoly(5, Square); { Open square }
    5:
      FillPoly(4, Triangle); { Solid triangle }
    6:
      DrawPoly(4, Triangle); { Open triangle }
    7:
      begin { + }
        Line(Xp, Yp1, Xp, Yp2);
        Line(Xp1, Yp, Xp2, Yp);
...
begin
  ...
  Rectangle(XmaxPixel + Round(0.02 * GetMaxX), YminPixel,
    XmaxPixel + Round(0.12 * GetMaxX) + Lmax, YminPixel + (N + 1) * CharHeight);
  ...
  begin
    ...
    begin
      SetLineStyle(Pred(CurvParam^[I].LineParam.Style), 0,
        CurvParam^[I].LineParam.Width);
...
procedure ConRec(Nx, Ny, Nc: Integer; X, Y, Z: PVector; F: PMatrix);
