...
function ChiSquareProbability(ChiSquare: Float;
  DegreesOfFreedom: longint): Float;
function ChiSquareProbabilityCompl(ChiSquare: Float;
  DegreesOfFreedom: longint): Float;
...
begin
  ...
  L := LnGamma(A + B) - LnGamma(A) - LnGamma(B) + (A - 1.0) * Ln(X) + (B - 1.0)
    * Ln(1.0 - X);
...
begin
  ...
  L := LnGamma(P) - LnGamma(Q) - 0.5 * Ln(Nu * Pi) - P * Ln(1.0 + X * X / Nu);
...
begin
  ...
  P2 := 0.5 * Nu2;
  S := P1 + P2;
  L := LnGamma(S) - LnGamma(P1) - LnGamma(P2) + P1 * Ln(R) + (P1 - 1.0) * Ln(X)
    - S * Ln(1.0 + R * X);
...
function ChiSquareProbability(ChiSquare: Float;
  DegreesOfFreedom: longint): Float;
begin
  result := GammP(DegreesOfFreedom / 2, ChiSquare / 2);
end;

function ChiSquareProbabilityCompl(ChiSquare: Float;
  DegreesOfFreedom: longint): Float;
begin
  result := GammQ(DegreesOfFreedom / 2, ChiSquare / 2);
end;
