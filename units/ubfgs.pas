...
uses
  utypes, ulinmin, ucompvec, unlfit, umachar, uConstants;

type
  TBFGS = class(TBaseOptAlgo)
  public
    ...
    procedure BFGS(Func: TFuncNVar; Gradient: TGradient; var X: TVector;
      Lb, Ub: Integer; MaxIter: Integer; Tol: Float; out F_min: Float;
      out G: TVector; out H_inv: TMatrix);
    { ------------------------------------------------------------------
      Minimization of a function of several variables by the
...
procedure TBFGS.SaveBFGS(FileName: string);
begin
...
procedure TBFGS.BFGS(Func: TFuncNVar; Gradient: TGradient; var X: TVector;
  Lb, Ub: Integer; MaxIter: Integer; Tol: Float; out F_min: Float;
  out G: TVector; out H_inv: TMatrix);
var
  I, J, Iter: Integer;
...
begin
  ...
  repeat
    ...
    for I := Lb to Ub do
      for J := Lb to Ub do
        H_inv[I, J] := H_inv[I, J] + R1dX[I] * dX[J] - R2HdG[I] * HdG[J] +
          P2U[I] * U[J];
