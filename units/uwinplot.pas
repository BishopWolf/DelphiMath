...
procedure SetWindow(Canvas: TCanvas; X1, X2, Y1, Y2: Integer;
  GraphBorder: Boolean);
{ ------------------------------------------------------------------
...
procedure SetLineParam(CurvIndex: Integer; Style: TPenStyle; Width: Integer;
  Color: TColor);
{ ------------------------------------------------------------------
...
procedure PlotCurveWithErrorBars(Canvas: TCanvas; X, Y, S: TVector;
...
procedure PlotFunc(Canvas: TCanvas; Func: TFunc; XMin, XMax: Float;
  Npt, CurvIndex: Integer); overload;
procedure PlotFunc(Canvas: TCanvas; Func: String; XMin, XMax: Float;
  Npt, CurvIndex: Integer); overload;
{ ------------------------------------------------------------------
  Plots a function
  ------------------------------------------------------------------
  ...
  or must contain a valid string representation 'f(x)'
  of a function of type y=f(x)
  ------------------------------------------------------------------ }

procedure WriteLegend(Canvas: TCanvas; NCurv: Integer;
...
const
  CurvColor: array [1 .. MaxCurvColor] of TColor = (clRed, clGreen, clBlue,
    clFuchsia, clAqua, clLime, clNavy, clOlive, clPurple);
...
begin
  ...
  begin
    if (XAxis.Scale = LinScale) and (Abs(X) < Eps) then
      X := 0.0;
  ...
  if XTitle <> '' then
    Canvas.TextOut(XminPixel + (XmaxPixel - XminPixel - Canvas.TextWidth(XTitle)
      ) div 2, YmaxPixel + 4 * TickLength, XTitle);
end;
...
begin
  ...
  for I := 0 to N do
  begin
    if (YAxis.Scale = LinScale) and (Abs(Y) < Eps) then
      Y := 0.0;
...
procedure SetPointParam(CurvIndex, Symbol, Size: Integer; Color: TColor);
begin
  if (CurvIndex < 1) or (CurvIndex > MaxCurv) then
    Exit;
...
function CheckPoint(X, Y: Float; var Xp, Yp: Integer): Boolean;
...
procedure PlotSymbol(Canvas: TCanvas; Xp, Yp, CurvIndex: Integer);
var
...
begin
  ...
  with Canvas do
    case CurvParam[CurvIndex].PointParam.Symbol of
      0:
        Pixels[Xp, Yp] := Brush.Color;
      1, 2:
        Ellipse(Xp1, Yp1, Xp2, Yp2); { Circle }
      3, 4:
        Rectangle(Xp1, Yp1, Xp2, Yp2); { Square }
      5, 6:
        Polygon([Point(Xp1, Yp2 - 1), Point(Xp2, Yp2 - 1), Point(Xp, Yp1 - 1)]);
      { Triangle }
      7:
        begin { + }
...
begin
  if XAxis.Scale = LogScale then
    X := Log10(X);
  if YAxis.Scale = LogScale then
    Y := Log10(Y);

  if not CheckPoint(X, Y, Xp, Yp) then
    Exit;
...
procedure PlotErrorBar(Canvas: TCanvas; Y, S: Float; Ns, Xp, Yp, Size: Integer);
...
procedure GenPlotCurve(Canvas: TCanvas; X, Y, S: TVector;
  Ns, Lb, Ub, CurvIndex: Integer; ErrorBars: Boolean);
...
procedure PlotCurve(Canvas: TCanvas; X, Y: TVector; Lb, Ub: Integer;
  CurvIndex: Integer);
begin
  GenPlotCurve(Canvas, X, Y, nil, 0, Lb, Ub, CurvIndex, False);
end;

procedure PlotCurveWithErrorBars(Canvas: TCanvas; X, Y, S: TVector; Ns: Integer;
  Lb, Ub: Integer; CurvIndex: Integer);
begin
...
var
  ...
  I: Integer;
begin
  if (Npt < 2) or (CurvParam[CurvIndex].LineParam.Style = psClear) then
    Exit;
...
begin
  ...
  Canvas.Rectangle(XmaxPixel + Round(0.02 * GraphWidth), YminPixel,
    XmaxPixel + Round(0.12 * GraphWidth) + Lmax, YminPixel + (N + 1) *
    CharHeight);
