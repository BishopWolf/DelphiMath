...
function Student_Distribution(t: Float; v: longint): Float;
function F_Distribution(F: Float; v1, v2: longint): Float;
function Cumulative_Binomial_Distribution(P: Float; K, N: longint): Float;
{ Distributions dependent from incomplete beta function }
...
begin
  if v < 1 then
    result := DefaultVal(FDomain, 0.0)
  else
    result := 1 - IBeta(0.5 * v, 0.5, v / (v + t * t));
end;

function F_Distribution(F: Float; v1, v2: longint): Float;
begin
  result := IBeta(0.5 * v2, 0.5 * v1, v2 / (v2 + v1 * F));
end;

function Cumulative_Binomial_Distribution(P: Float; K, N: longint): Float;
begin
  result := IBeta(K, N - K + 1, P);
end;
