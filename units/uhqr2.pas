...
procedure Hqr2(H: TMatrix; Lb, Ub, I_low, I_igh: Integer;
  out Lambda: TCompVector; var Z: TMatrix);
...
{ ------------------------------------------------------------------
  ...
  Lambda contain the real and imaginary parts, respectively, of
...
270: { One root found }
  H[En, En] := X + T;
  Lambda[En].Real := H[En, En];
  Lambda[En].Imaginary := 0.0;
  En := Na;
  goto 60;
...
280: { Two roots found }
  ...
  Zz := P + DSgn(Zz, P);
  Lambda[Na].Real := X + Zz;
  Lambda[En].Real := Lambda[Na].Real;
  if Zz <> 0.0 then
    Lambda[En].Real := X - W / Zz;
  Lambda[Na].Imaginary := 0.0;
  Lambda[En].Imaginary := 0.0;
  X := H[En, Na];
  S := Abs(X) + Abs(Zz);
  P := X / S;
...
320: { Complex pair }
  Lambda[Na].Real := X + P;
  Lambda[En].Real := Lambda[Na].Real;
  Lambda[Na].Imaginary := Zz;
  Lambda[En].Imaginary := -Zz;
...
340:
  ...
  600: { Real vector }
    ...
    begin
      ...
      if Lambda[I].Imaginary >= 0.0 then
        goto 630;
      Zz := W;
      S := R;
      goto 700;
    ...
    640: { Solve X equations }
      X := H[I, I + 1];
      Y := H[I + 1, I];
      Q := Sqr(Lambda[I].Real - P) + Sqr(Lambda[I].Imaginary);
      T := (X * S - Zz * R) / Q;
      H[I, En] := T;
  ...
  710:
    ...
    780:
      X := H[I, I + 1];
      Y := H[I + 1, I];
      Vr := Sqr(Lambda[I].Real - P) + Sqr(Lambda[I].Imaginary) - Sqr(Q);
      Vi := (Lambda[I].Real - P) * 2.0 * Q;
      ...
      Cdiv(X * R - Zz * Ra + Q * Sa, X * S - Zz * Sa - Q * Ra, Vr, Vi, H[I, Na],
        H[I, En]);
