...
procedure RKF45(F: TDiffEqs; Neqn: Integer; Y, Yp: TVector;
  var T, RelErr: Float; Tout, AbsErr: Float; var Flag: Integer);

implementation

var
  maxeqn, flag_save, init, kflag, kop, nfe: Integer;
  relerr_save, abserr_save, h: Float;
  f1, f2, f3, f4, f5: TVector;

procedure Fehl(F: TDiffEqs; Neqn: Integer; Y: TVector; T, h: Float; Yp: TVector;
  out f1, f2, f3, f4, f5, S: TVector);
{ ------------------------------------------------------------------
...
procedure RKF45(F: TDiffEqs; Neqn: Integer; Y, Yp: TVector;
  var T, RelErr: Float; // if relerr isn't a var see below
  Tout, AbsErr: Float; var Flag: Integer);
{ ------------------------------------------------------------------
  ...
  * 8, invalid input parameters, indicates one of the following:
  ...
  Modified thursday, 09/27/2007 by Alex Vergara Gil

  relerr put as var input because it cannot be outputed without this
  reason, now the esence of the procedure is complete.
  ------------------------------------------------------------------ }
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        exit
      else if (kflag = 6) and (RelErr <= relerr_save) and (AbsErr <= abserr_save)
      then
        exit;
    end
  ...
  begin
    RelErr := relerr_min; // This does nothing if relerr is not a var input!!!
    Flag := 3;
    kflag := 3;
    exit
  end;
  ...
  begin
    ...
    h := math.Max(h, 26 * MachEp * math.Max(abs(T), abs(dt)));
