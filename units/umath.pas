...
{$ENDIF}{ Exponential }
function Exp2(X: Float): Float; {$IFDEF INLININGSUPPORTED} inline; {$ENDIF}{ 2^X }
function Exp10(X: Float): Float; {$IFDEF INLININGSUPPORTED} inline; {$ENDIF}{ 10^X }
function Log(X: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ Natural log }
function Log2(X: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ Log, base 2 }
function Log10(X: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ Decimal log }
function LogA(X, A: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ Log, base A }
function IntPower(X: Float; N: Integer): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ X^N }
function Power(X, Y: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ X^Y, X >= 0 }
function Atan(CatOp, CatAdy: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ Arcotangente con chequeos }
function Divide(X, Y: Float): Float; {$IFDEF INLININGSUPPORTED} inline;
{$ENDIF}{ División segura }

implementation

function Atan(CatOp, CatAdy: Float): Float;
begin
  if CatAdy <> 0 then
  begin
    result := ArcTan2(CatOp, CatAdy);
    if result < 0 then
      result := result + 2 * pi;
  end
  else
...
begin
  ...
  if MathErr = FOk then
    if A = 1.0 then
      Y := DefaultVal(FSing, Sign(Y) * MaxNum)
    else
      Y := Y / Log(A);
  LogA := Y;
end;
...
begin
  if IsZero(X, MachEp) then
  begin
    PowerTests := True;
    if IsZero(Y, MachEp) then { 0^0 = lim  X^X = 1 }
      Res := 1.0 { X->0 }
    else if Y > 0.0 then
      Res := 0.0 { 0^Y = 0 }
    else
...
begin
  ...
  if Invert then
    if Abs(T) < InverseMaxNum then { Only here overflow }
      T := DefaultVal(FOverflow, Sign(T) * MaxNum)
    else
      T := 1.0 / T;
...
function Divide(X, Y: Float): Float;
{ División segura }
begin
  if IsInfinite(Y) or IsZero(X) then
    result:=0
  else if Y <> 0 then
    result := X / Y
  else
