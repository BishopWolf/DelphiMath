...
uses
  SysUtils, Types, Classes, Math, uBaseGeometry;
...
type
  ...
  TIntPolygon = array of TIntPoint;
  TIntPolygons = array of TIntPolygon;
  ...
  public
    ...
    function AddPolygon(const polygon: TIntPolygon;
      PolyType: TPolyType): Boolean;
    function AddPolygons(const polygons: TIntPolygons;
      PolyType: TPolyType): Boolean;
  ...
  private
    ...
    procedure IntersectEdges(E1, E2: PEdge; const Pt: TIntPoint;
    ...
    function GetResult: TIntPolygons;
  ...
  public
    function Execute(clipType: TClipType; out solution: TIntPolygons;
...
function Orientation(const Pts: TIntPolygon): Boolean; overload;
function Area(const Pts: TIntPolygon): Double; overload;
function Area(const Pts: TIntPolygon; out Center: TIntPoint): Double; overload;
function ReversePolygon(const Pts: TIntPolygon): TIntPolygon;
function ReversePolygons(const Pts: TIntPolygons): TIntPolygons;
...
function OffsetPolygons(const Polys: TIntPolygons; const Delta: Double;
  JoinType: TJoinType = jtSquare; MiterLimit: Double = 2;
  ChecksInput: Boolean = true): TIntPolygons;

// SimplifyPolygon converts a self-intersecting polygon into a simple polygon.
function SimplifyPolygon(const poly: TIntPolygon;
  FillType: TPolyFillType = pftEvenOdd): TIntPolygons;
function SimplifyPolygons(const Polys: TIntPolygons;
  FillType: TPolyFillType = pftEvenOdd): TIntPolygons;
...
begin
  ...
  if Int64Rec(Result.Lo).Hi < Int64Rec(Int1.Lo).Hi then
    inc(Result.Hi);
end;
// ------------------------------------------------------------------------------
...
begin
  ...
  Result.Lo := A + B;
  if Int64Rec(Result.Lo).Hi < Int64Rec(A).Hi then
    inc(Result.Hi);
...
begin
  ...
  begin
    ...
    if Lo = 0 then
      ...
      Result := -(not Val.Hi * shift64 + Int64Rec(Lo).Hi * shift32 +
        Int64Rec(Lo).Lo);
  end
  else
    Result := Val.Hi * shift64 + Int64Rec(Val.Lo).Hi * shift32 +
      Int64Rec(Val.Lo).Lo;
end;
// ------------------------------------------------------------------------------
...
function FullRangeNeeded(const Pts: TIntPolygon): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function Orientation(const Pts: TIntPolygon): Boolean;
var
...
function Orientation(OutRec: POutRec; UseFullInt64Range: Boolean)
  : Boolean; overload;
var
...
begin
  ...
  if FullRangeNeeded(Pts) then
  begin
    tA := (Int128Sub(Int128Mul(Pts[HighI].X, Pts[0].Y), Int128Mul(Pts[0].X,
      Pts[HighI].Y))).Lo;
    resX128 := Int128Mul(tA, Pts[HighI].X + Pts[0].X);
    resY128 := Int128Mul(tA, Pts[HighI].Y + Pts[0].Y);
    A := Int128(tA);
    for I := 0 to HighI - 1 do
    begin
      tA := (Int128Sub(Int128Mul(Pts[I].X, Pts[I + 1].Y),
        Int128Mul(Pts[I + 1].X, Pts[I].Y))).Lo;
      A := Int128Add(A, Int128(tA));
      resX128 := Int128Mul(tA, Pts[I].X + Pts[I + 1].X);
      resY128 := Int128Mul(tA, Pts[I].Y + Pts[I + 1].Y);
    end;
    Result := Int128AsDouble(A) / 2;
    Center := IntPoint(round(Int128AsDouble(resX128) / (Int128AsDouble(A) * 3)),
      round(Int128AsDouble(resY128) / (Int128AsDouble(A) * 3)));
  end
  else
  begin
    D := (Pts[HighI].X * Pts[0].Y) - (Pts[0].X * Pts[HighI].Y);
    ...
    Result := D / 2;
    Center := IntPoint(round(resX / (3 * D)), round(resy / (3 * D)));
  end;
end;
// ------------------------------------------------------------------------------
...
function PointInPolygon(const Pt: TIntPoint; PP: POutPt;
  ...
  begin
    repeat
      ...
      begin
        ...
        B := Int128Div(Int128Mul(Pp2.Prev.Pt.X - Pp2.Pt.X, Pt.Y - Pp2.Pt.Y),
          Int128(Pp2.Prev.Pt.Y - Pp2.Pt.Y));
        if Int128LessThan(A, B) then
          Result := not Result;
      end;
      Pp2 := Pp2.Next;
    until Pp2 = PP;
...
function SlopesEqual(E1, E2: PEdge; UseFullInt64Range: Boolean)
  : Boolean; overload;
begin
  if UseFullInt64Range then
...
function SlopesEqual(const Pt1, Pt2, Pt3: TIntPoint; UseFullInt64Range: Boolean)
  : Boolean; overload;
begin
  if UseFullInt64Range then
...
function IntersectPoint(Edge1, Edge2: PEdge; out ip: TIntPoint;
  UseFullInt64Range: Boolean): Boolean; overload;
var
  B1, B2, M: Double;
begin
...
function TClipperBase.AddPolygon(const polygon: TIntPolygon;
  PolyType: TPolyType): Boolean;
...
begin
  ...
  begin
    ...
    if PointsEqual(Pg[J], polygon[I]) then
      Continue
    else if (J > 0) and SlopesEqual(Pg[J - 1], Pg[J], polygon[I], FUse64BitRange)
    then
    begin
      if PointsEqual(Pg[J - 1], polygon[I]) then
        dec(J);
    end
    else
      inc(J);
  ...
  begin
    ...
    if PointsEqual(Pg[J], Pg[0]) then
      dec(J)
    else if PointsEqual(Pg[0], Pg[1]) or SlopesEqual(Pg[J], Pg[0], Pg[1],
      FUse64BitRange) then
    begin
      Pg[0] := Pg[J];
      dec(J);
    end
    else if SlopesEqual(Pg[J - 1], Pg[J], Pg[0], FUse64BitRange) then
      dec(J)
...
function TClipperBase.AddPolygons(const polygons: TIntPolygons;
  PolyType: TPolyType): Boolean;
var
  I: Integer;
begin
...
function TClipper.Execute(clipType: TClipType; out solution: TIntPolygons;
  subjFillType: TPolyFillType = pftEvenOdd;
    ...
    try
      ...
      begin
        ...
        if (OutRec.BottomPt = OutRec.BottomFlag) and
          (Orientation(OutRec, FUse64BitRange) <> (Area(OutRec, FUse64BitRange)
          > 0)) then
...
begin
  ...
  if assigned(prevE) and (prevE.OutIdx >= 0) and
    (TopX(prevE, Pt.Y) = TopX(E, Pt.Y)) and SlopesEqual(E, prevE, FUse64BitRange)
  then
    AddJoin(E, prevE);
end;
// ------------------------------------------------------------------------------
...
procedure TClipper.IntersectEdges(E1, E2: PEdge; const Pt: TIntPoint;
  protects: TIntersectProtects = []);
...
begin
  ...
  begin
    if HorzEdge.OutIdx >= 0 then
      IntersectEdges(HorzEdge, eMaxPair, IntPoint(HorzEdge.XTop,
  ...
  begin
    ...
    begin
      ...
      if assigned(ePrev) and (ePrev.XCurr = E.XBot) and (ePrev.YCurr = E.YBot)
        ...
        (ePrev.YCurr > ePrev.YTop) and SlopesEqual(E, ePrev, FUse64BitRange)
      then
      begin
        ...
        (eNext.YCurr > eNext.YTop) and SlopesEqual(E, eNext, FUse64BitRange)
      then
      begin
...
function TClipper.GetResult: TIntPolygons;
var
  I, J, K, Cnt: Integer;
  OutRec: POutRec;
  Op: POutPt;
...
begin
  ...
  begin
    ...
    if PointsEqual(PP.Pt, PP.Next.Pt) or SlopesEqual(PP.Prev.Pt, PP.Pt,
      PP.Next.Pt, FUse64BitRange) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if PointInPolygon(OutRec1.Pts.Pt, OutRec2.Pts, FUse64BitRange) then
      begin
      ...
      if (Orientation(OutRec1, FUse64BitRange) <> (Area(OutRec1, FUse64BitRange)
        >= 0)) then
        DisposeBottomPt(OutRec1);
      if (Orientation(OutRec2, FUse64BitRange) <> (Area(OutRec2, FUse64BitRange)
        >= 0)) then
...
function BuildArc(const Pt: TIntPoint; A1, A2, R: Single): TIntPolygon;
var
  I, N: Integer;
  A, D: Double;
  Steps: Int64;
...
function GetBounds(const Pts: TIntPolygons): TIntRect;
var
  I, J: Integer;
begin
  with Result do
