...
uses
  uConstants, ugamma, uigamma, uinvnorm;
...
function Inv_Cauchy_CumDist(y, mean, HalfWidth: Float): Float;
{ ------------------------------------------------------------------
  Inverse of Cauchy Cumulative distribution function

  Returns the argument, X, for which the area under the Cauchy
  probability density function (integrated from 0 to X)
  is equal to P.
  ------------------------------------------------------------------ }
...
begin
  FWdiv2 := (HalfWidth / 2);
  tanpiy := Tan(PI * y);
  result := mean + FWdiv2 * (tanpiy - mean / FWdiv2) /
    (1 + mean * tanpiy / FWdiv2);
  // result:=mean*(1-Cosh(PI*y))+Pythag(FWdiv2,mean)*Sinh(PI*y);  This is also the solution but doesn't work
end;
