...
procedure Jacobi(A: TMatrix; Lb, Ub, MaxIter: Integer; Tol: Float;
  out Lambda: TVector; out V: TMatrix);
{ ------------------------------------------------------------------
...
begin
  ...
  repeat
    R := 0.0;
    Im := 0;
    Jm := 0; // avoiding the not initialized warning
  ...
  DQSortMatrixRowsByVector(V, Lambda, Ub, Ub, Lb, Ub);
  { for I := Lb to Pred(Ub) do
    begin
    K := I;
    R := Lambda[I];
