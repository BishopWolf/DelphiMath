...
uses
  utypes, math, uminmax, utrigo, ulu, uConstants;

type
  TSVD = class(TBaseDecomp)
  private
    A: TMatrix;
    Lb, Ub1, Ub2: Integer;
  ...
  public
    S: TVector;
    V: TMatrix;
    constructor Create(lA: TMatrix; lLb, lUb1, lUb2: Integer);
    destructor Destroy; override;
    function Solve(const B: TVector): TVector; override;
    function _Approx: TMatrix;
    { ------------------------------------------------------------------
    ...
    { ------------------------------------------------------------------
      ...
      Ub  = index of last vector element = Ub2
      ------------------------------------------------------------------
...
constructor TSVD.Create(lA: TMatrix; lLb, lUb1, lUb2: Integer);
begin
  inherited Create;
  Lb := lLb;
  Ub1 := lUb1;
...
procedure TSVD._SetZero(Tol: Float);
var
  Threshold: Float;
  I: Integer;
begin
  Threshold := S[Lb];
  for I := Lb + 1 to Ub2 do
...
function TSVD._Solve(const B: TVector): TVector;
var
  I, J, K: Integer;
  Sum: Float;
  Tmp: TVector;
...
function TSVD.Solve(const B: TVector): TVector;
begin
  Result := _Solve(B);
end;

function TSVD._Approx: TMatrix;
var
  I, J, K: Integer;
begin
  ...
  for I := Lb to Ub1 do
    for J := Lb to Ub2 do
    begin
      // Result[I,J] := 0.0; // not need
      for K := Lb to Ub2 do
        if S[K] > 0.0 then
          Result[I, J] := Result[I, J] + A[I, K] * V[J, K];
    end;
end;
