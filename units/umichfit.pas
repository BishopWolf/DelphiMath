...
uses
  utypes, ulinfit, unlfit, umachar, uconstants;

procedure MichFit(X, Y: TVector; Lb, Ub: Integer; MaxIter: Integer; Tol: Float;
...
procedure WMichFit(X, Y, S: TVector; Lb, Ub: Integer; MaxIter: Integer;
...
{ ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------ }
begin
  MichFit_Func := B[1] * X / (B[2] + X);
end;

procedure MichFit_Deriv(X, Y: Float; B: TVector; out D: TVector);
{ ------------------------------------------------------------------
...
begin
  DimVector(D, 2);
  D[1] := Y / B[1]; { dy/dYmax = x / (Km + x) }
  D[2] := -Y / (B[2] + X); { dy/dKm = - Ymax.x / (Km + x)^2 }
end;
...
procedure GenMichFit(Mode: TRegMode; X, Y, S: TVector; Lb, Ub: Integer;
...
begin
  ...
  case Mode of
    OLS:
      NLFit(MichFit_Func, MichFit_Deriv, X, Y, Lb, Ub, MaxIter, Tol, B,
        FirstParam, LastParam, V);
    WLS:
      WNLFit(MichFit_Func, MichFit_Deriv, X, Y, S, Lb, Ub, MaxIter, Tol, B,
        FirstParam, LastParam, V);
  end;
end;
